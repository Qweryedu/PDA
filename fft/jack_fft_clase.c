...
int main (int argc, char *argv[]) {
  ...
  freq = (double *) malloc(sizeof(double) * nframes);
  freq[0] = 0.0;
  freq[nframes/2] = sample_rate/2;
  double freq_diff = sample_rate/nframes;
  for(int i =1; i < nframes/2; ++i) {
	  freq[i] = freq_diff*i;
	  freq[nframes-1] = -freq[i];
  }
