...
int jack_callback (jack_nframes_t nframes, void *arg){
  ...
  jack_default_audio_sample_t *in2, *out2;
  ...
  for(int i = 0; i < nframes; ++i){
  	out[i] = buff[buff_index];
  	buff[buff_index] = in[i];
  	++buff_index;
...
int main (int argc, char *argv[]) {
	...
	if (argc < 2){
		printf ("Necesito numero de segundos para desfasar.\n");
		exit(1);
	}
	
	buff_size = atoi(argv[1]);
  printf("%d", sample_rate);
	// Multiplicamos el tamaño recuperado del buffer en segundos por
  // por la frecuencia de muestreo y para evitar el redondeo
  buff_size = (int)(buff_size * sample_rate);

	if (buff_size <= 0){
		printf ("El numero de segundos tiene que ser mayor a cero.\n");
		exit(1);
	}
		
	printf ("Desfasando %d segundos.\n",buff_size);
  printf ("O %d muestras\n", buff_size/sample_rate);
	
	buff = malloc(buff_size*sizeof(jack_default_audio_sample_t));

  const char *client_name = "delay_seg";
  ...
  printf ("Engine sample rate: %d\n", sample_rate);
	printf ("Desafasando %f segundos.\n",(double)buff_size/sample_rate);
  ...
  if ((input_port == NULL) || (output_port == NULL) || (input_port2 == NULL) || (output_port2 == NULL)) {
